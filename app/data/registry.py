...
def _ingest_delimited_source(
    ...
    *,
    ...
    safe_name = Path(filename or f"upload.{source_format}").name
    ...
    raw_table_name = Path(safe_name).stem
    ...
    relation = _schema_relation_for_frame(
        ...
        join_keys=[],
        lineage={"format": source_format, "json_path": "$", "raw_name": raw_table_name},
...
def _ingest_json_source(filename: str, content: bytes, source_id: str | None = None) -> SourcePackage:
    ...
    raw_root_name = Path(safe_name).stem
    ...
    relation_raw_names: dict[tuple[str, ...], str] = {(): raw_root_name}
    ...
    def handle_array(
        ...
        *,
        ...
        relation_raw_names[array_path] = f"{raw_root_name}.{'.'.join(full_path)}"
    ...
    relations: list[RelationPackage] = [
        RelationPackage(
            relation=_schema_relation_for_frame(
                ...
                parent_relation=None,
                join_keys=[],
                lineage={"format": "json", "json_path": "$", "raw_name": relation_raw_names[()]},
        ...
        relations.append(
            RelationPackage(
                relation=_schema_relation_for_frame(
                    ...
                    lineage={
                        "format": "json",
                        "json_path": ".".join(relation_path),
                        "raw_name": relation_raw_names.get(relation_path, relation_name),
                    },
